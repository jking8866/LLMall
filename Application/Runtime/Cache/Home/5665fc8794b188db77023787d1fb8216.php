			...
			<div class="col-md-12">
				...
				<div class="nav-tabs-custom">
					<ul class="nav nav-tabs">
						<li ><a href="/zy/Order/add"  >集体班</a></li>
						<li class="active"><a href="#tab_1"  data-toggle="tab" >一对多</a></li>
							...
							<div class="row">
								<div class="col-md-6">
									...
									<div class="">
										...
										<!-- /.box-header -->
										<!-- form start -->
										<form role="form" method="post" action="/zy/Order/insertClazzMore">
											<div class="box-body">
												...
												<div class="form-group">
							                      <label for="hour_count">报名小时数</label>
							                      <input type="number" min="0" class="form-control" name="hour_count" id="hour_count" onchange="getOrderInfo()" >
							                    </div>
							                    
												<div class="form-group">
													<label for="grade_id">年级</label> <select
														class="form-control" id="grade_id" name="grade_id"  onchange="getOrderInfo()" >
														<option>请填写</option>
														<?php if(is_array($GradeList)): $i = 0; $__LIST__ = $GradeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["grade_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
													</select>
												</div>
												<div class="form-group">
													<label for="teacher_level">教师等级</label> 
													<select class="form-control" id="teacher_level" name="teacher_level"  onchange="getOrderInfo()" >
														<option>请填写</option>
														<option value="1">普通</option>
														<option value="2">中级</option>
														<option value="3">高级</option>
														<option value="4">五所</option>
													</select>
												</div>
												
												<div class="form-group">
													<label for="one_to_more_type">一对多类型</label> 
													<select class="form-control" id="one_to_more_type" name="one_to_more_type"  onchange="getOrderInfo()" >
														<option>请填写</option>
														<option value="1">一对二</option>
														<option value="2">一对三</option>
														<option value="3">一对四</option>
														<option value="4">一对五</option>
													</select>
												</div>
										...
										<div class="row invoice-info">
											<div class="col-sm-12 invoice-col">
											 ...
											 <div class="col-sm-12 invoice-col">
											 	合计
												<address>
													   报 <span id="order_hour_count"></span> 小时
													   <br> 每小时 <span id="order_each_cost"></span> 元
													   <br> 共<span id= "order_cost_all" ></span>元
												</address>
											</div>
											<!-- /.col -->
										</div>
...
function getOrderInfo(){
	...
	$.ajax({
		type: "get",
		url : "/zy/CostMore/ajax_getCostMoreInfo",
		data :  {"grade_id":$("#grade_id").val(),"teacher_level":$("#teacher_level").val(),"one_to_more_type":$("#one_to_more_type").val()},
		dataType : "json",
		async: false,
		success : function( obj ){
			 $("#order_hour_count").html($("#hour_count").val());
			 $("#order_each_cost").html(obj.each_cost);
			 $("#order_cost_all").html(    $("#hour_count").val() *   obj.each_cost   );
		},
		error : function( XMLHttpRequest, textStatus, errorThrown ){
		},
		complete : function (XMLHttpRequest, textStatus) {
